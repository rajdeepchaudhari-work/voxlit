  ...
  return (
    <div className="animate-onboarding-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 0 }}>
      ...
      {status === 'not-determined' && (
        <PrimaryButton onClick={requestMic}>Grant microphone access</PrimaryButton>
      )}
      {status === 'denied' && (
        <PrimaryButton onClick={() => ipc.requestPermission('microphone')}>Open System Settings</PrimaryButton>
      )}
      {status === 'granted' && (
        <PrimaryButton onClick={onNext} disabled={false}>Continue →</PrimaryButton>
      )}
...
function DoneStep({ settings, onFinish }: { settings: VoxlitSettings | null; onFinish: () => void }) {
  ...
  return (
    <div className="animate-onboarding-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 0 }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `2px solid ${active ? '#0A0A0A' : '#0A0A0A'}`,
      ...
      <PrimaryButton onClick={onFinish}>Launch Voxlit →</PrimaryButton>
    </div>
  )
}
